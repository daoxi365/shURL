...
function make() {
	try {
		...
		if (!pattern.test(agoURL)) {
			setNote("<b class=\"text-purple-700\">您的输入似乎不是 URL，请重新输入。</b>");
			return;
		}
		...
		query.find().then((link) => {
			if (link.length >= 1) {
				...
				var shURL = `${WEBSITE}/` + (GITHUB ? `${GITHUB_REPOSITORY}/` : ``) + `?x=${shLink}`;
				setNote(`<b><a href="${shURL}" class="text-blue-700 hover:text-blue-800"><code>${shURL}</code></a></b>`);
				return;
			}
			else {
				...
				todo.save().then(
					...
					(error) => {
						setNote(`<b class="text-rose-600">发生错误（<code>CODE = ${error.code}</code>），请稍后再试。</b>`);
					}
				);
			}
		});
		...
		setNote(`<b class="text-rose-600">发生错误（<code>${err}</code>），请稍后再试。</b>`);
	}
}
...
function getName(name) {
    ...
    reg = null; 
    r = null; 
    return context == null || context == "" || context == "undefined" ? "ERROR ERROR ERROR!!!" : context;
}
...
function loadTo() {
	try {
		...
		query.find().then((link) => {
			if (!link.length) setNote(`<b class="text-rose-600">发生错误，此链接尚未被注册或在数据库中被删除。</b>`);
...
try {
	...
	initial.save().then(
		(todo) => {},
		(error) => {
			setNote(`<b class="text-rose-600">初始化失败（<code>CODE = ${err.code}</code>），请联系网站开发者。</b>`);
		}
	);
}
